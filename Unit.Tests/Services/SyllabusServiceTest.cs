...
namespace SyllabusService.Tests.Services
{
    public class SyllabusServiceTest 
    {
        ...
        [Fact]
        public async Task UpdateSyllabus_ShouldReturnBadRequest_WhenInvalidRequestName()
        {
        ...
        [Fact]
        public async Task UpdateSyllabus_ShouldReturnBadRequest_WhenInvalidRequestDescription()
        {
            var request = new UpdateSyllabusRequest(1, "PRM", "", 500000, 5, "");

            var result = await _service.UpdateSyllabusAsync(request);

            result.StatusResponseCode.Should().Be("badRequest");
            result.Message.Should().Contain("must not be empty.");
        }

        [Fact]
        public async Task UpdateSyllabus_ShouldReturnBadRequest_WhenInvalidCostSyllabus()
        {
            var request = new UpdateSyllabusRequest(1, "PRM", "se", 50000, 5, "");
        ...
        [Fact]
        public async Task UpdateSyllabus_ShouldReturnBadRequest_WhenHoursOfSyllabusLessThan10()
        {
            var request = new UpdateSyllabusRequest(1, "PRM", "se", 500000, 5, "");

            var result = await _service.UpdateSyllabusAsync(request);

            result.StatusResponseCode.Should().Be("badRequest");
            result.Message.Should().Contain("Hours of syllabus");
        }
        [Fact]
        public async Task UpdateSyllabus_ShouldReturnBadRequest_WhenIsActiveEmpty()
        {
            var request = new UpdateSyllabusRequest(1, "PRM", "se", 500000, 12, "");

            var result = await _service.UpdateSyllabusAsync(request);

            result.StatusResponseCode.Should().Be("badRequest");
            result.Message.Should().Contain("Is Active must not be empty.");
        }
        [Fact]
        public async Task UpdateSyllabus_ShouldReturnBadRequest_WhenIsActiveInvalid()
        {
            var request = new UpdateSyllabusRequest(1, "PRM", "se", 500000, 12, "fa");

            var result = await _service.UpdateSyllabusAsync(request);

            result.StatusResponseCode.Should().Be("badRequest");
            result.Message.Should().Contain("Is Active must be either 'true' or 'false'.");
        }
        [Fact]
        ...
        [Fact]
        public async Task UpdateSyllabus_ShouldReturnOk_WhenValid_IsActiveTrue()
        {
        ...
        [Fact]
        public async Task UpdateSyllabus_ShouldReturnOk_WhenValid_IsActiveFalse()
        {
            var request = new UpdateSyllabusRequest(1, "PRN231", "Web API", 250000, 25, "false");
        ...
        [Fact]
        public async Task GetAllActiveSyllabusAsync_ShouldReturnOk_WhenSyllabusExist()
        {
            // Arrange
            _mockRepo.Setup(x => x.GetAllActiveSyllabusAsync())
                .ReturnsAsync(new List<Syllabus>
                {
            new Syllabus
            {
            ...
            // Act
            var result = await _service.GetAllActiveSyllabusAsync();

            // Assert
            result.StatusResponseCode.Should().Be("ok");
            result.Message.Should().Contain("Get All Active syllabuses successfully");

            var data = result.Data.Should().BeAssignableTo<IEnumerable<SyllabusDTO>>().Subject.ToList();
            data.Should().HaveCount(2);

            data[0].Id.Should().Be(1);
            data[0].IsActive.Should().Be("true");
            data[1].IsActive.Should().Be("false");
        }
